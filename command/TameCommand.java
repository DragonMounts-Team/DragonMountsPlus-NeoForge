...
import static net.dragonmounts.plus.common.command.DMCommand.getSingleProfileOrException;
    ...
    public static int tame(CommandContext<CommandSourceStack> context, Collection<? extends Entity> targets, GameProfile owner, boolean forced) {
        ...
        if (flag) {
            if (targets.size() == 1) {
                ...
                source.sendFailure(Component.translatable("commands.dragonmounts.plus.tame.multiple", count, owner.getName()));
            }
        } else if (count == 1) {
            final var temp = cache;
            source.sendSuccess(() -> Component.translatable("commands.dragonmounts.plus.tame.single", temp.getDisplayName(), owner.getName()), true);
        } else {
            final var temp = count;
            source.sendSuccess(() -> Component.translatable("commands.dragonmounts.plus.tame.multiple", temp, owner.getName()), true);
        }
        return count;
    }
}
