    ...
    public static int free(CommandContext<CommandSourceStack> context, Collection<? extends Entity> targets, UUID owner, boolean forced) {
        ...
        if (flag) {
            if (targets.size() == 1) {
                ...
                source.sendFailure(Component.translatable("commands.dragonmounts.plus.free.multiple", count));
            }
        } else if (count == 1) {
            final var temp = cache;
            source.sendSuccess(() -> Component.translatable("commands.dragonmounts.plus.free.single", temp.getDisplayName()), true);
        } else {
            final var temp = count;
            source.sendSuccess(() -> Component.translatable("commands.dragonmounts.plus.free.multiple", temp), true);
        }
        return count;
    }
}
